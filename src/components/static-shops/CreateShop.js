...
import { Days, imageUpload } from "../../common/Common";
import FormInfo from "./FormInfo";
import FormValidation from "./FormValidation";
import axios from "axios";
import { bucketFolderNames } from "../../common/utils";
...
export default function CreateShop() {
  ...
  const { handleChange, handleFileChange, values, handleSubmit, errors } =
    FormInfo(FormValidation, selectedDays);

  // Working Days
  const handleShopSubmit = async (event) => {
    const data = handleSubmit(event);
    const { image: imageAsFile } = data;
    const image = await imageUpload(imageAsFile, bucketFolderNames.SHOPS);
    data.image = image;
    axios
      .post(
        "http://localhost:5001/khadim-tailors/us-central1/shops/addShop",
        data
      )
      .then((response) => {
        let type = "error";
  ...
  const checkHandler = (day, ind) => {
    ...
    if (thisChildStatus === daysChildIndexStatus && day.id !== "allDays") {
      if (selectedDays[ind].status && selectedDays[allDaysIndex].status) {
        ...
        selectedDays[ind].status &&
        !selectedDays[allDaysIndex].status
      ) {
        selectedDays[ind].status = false;
      } else {
        ...
        <div className="formSubContainer">
          ...
          <form className="row g-3">
            <div className="col-md-6">
              ...
              />
              {errors.shopName && (
                <p className="errorMessage fz-12 mb-0 mt-1">
                  {errors.shopName}
                </p>
              )}
            </div>
            <div className="col-md-6">
              ...
              />
              {errors.phone && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.phone}</p>
              )}
            </div>
            <div className="col-6">
              ...
              />
              {errors.email && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.email}</p>
              )}
            </div>
            <div className="col-md-3">
              ...
              />
              {errors.openAt && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.openAt}</p>
              )}
            </div>
            <div className="col-md-3">
              <label htmlFor="inputEmail4" className="form-label">
              ...
              />
              {errors.closeAt && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.closeAt}</p>
              )}
            </div>
                  ...
                  return (
                    <div className="form-check" key={day.id}>
                      ...
                      <input
                        ...
                        checked={day.status}
                        type="checkbox"
                        onClick={(e) => checkHandler(day, index)}
                      />
            ...
            <div className="col-12">
              ...
              />
              {errors.address && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.address}</p>
              )}
            </div>
            <div className="col-12">
            ...
            <div className="col-md-5">
              ...
              />
              {errors.city && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.city}</p>
              )}
            </div>
            <div className="col-md-4">
              ...
              />
              {errors.state && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.state}</p>
              )}
            </div>
            <div className="col-md-3">
              ...
              />
              {errors.zip && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.zip}</p>
              )}
            </div>
            <div className="col-12">
              <label htmlFor="inputZip" className="form-label">
              ...
              />
              {errors.map && (
                <p className="errorMessage fz-12 mb-0 mt-1">{errors.map}</p>
              )}
            </div>
